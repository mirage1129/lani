...
alphabet.unshift('Saved groups', 'Frequently invited')

type InviteListProp = {
  navigation: StackNavigationProp<any, 'Invite List'>
}

const InviteList = ({ navigation }: InviteListProp) => {
  const data = alphabet.map((groupName: any) => ({ title: groupName, data: ['Cedric Chin', 'Adam Wishney', 'Alex Nodelan'] }))

  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.actionArea}>
        <Text style={styles.actionBase} onPress={() => navigation.navigate('Event Detail')}>Cancel</Text>
        <Text style={[styles.actionBase, styles.inviteAction]} onPress={() => navigation.navigate('Event Detail')}>Invite</Text>
      </View>
          ...
          renderSectionHeader={({ section }) => (
            <View style={styles.sectionHeader}>
              ...
              {section.title == 'Saved groups' &&
                <Ionicons name='add-circle-outline' size={24} color='#333' style={styles.newGroups} onPress={() => navigation.navigate('Invite Group List')}/>
              }
            </View>
          )}
